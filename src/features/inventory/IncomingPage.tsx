...
const INCOMING_TYPE_OPTIONS = [
  ...
  { value: 'production', label: '생산입고' },
  { value: 'return', label: '반품입고' },
  { value: 'other', label: '기타입고' },
]
...
export default function IncomingPage() {
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm<IncomingForm>()
  ...
  const onSubmit = async (data: IncomingForm) => {
    setLoading(true)
    setResult(null)
    try {
      ...
      setResult({ success: true, message: '입고 처리가 완료되었습니다.' })
      queryClient.invalidateQueries({ queryKey: ['inventory'] })
      queryClient.invalidateQueries({ queryKey: ['inventoryTransactions'] })
      reset()
    } catch (err: unknown) {
      const message = (err as { message?: string }).message ?? '입고 처리에 실패했습니다.'
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-4">입고처리</h1>
      ...
      <Card title="입고 정보 입력">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Select
              label="입고유형 *"
              options={INCOMING_TYPE_OPTIONS}
              {...register('incomingType', { required: '유형을 선택하세요' })}
            />
          ...
          <div className="flex justify-end gap-2 pt-4 border-t">
            ...
            <Button type="submit" loading={loading}>입고 처리</Button>
          </div>
        </form>
      </Card>
    </div>
