...
exports.processInventoryTx = void 0;
 ...
 */
exports.processInventoryTx = (0, https_1.onCall)({ region: "asia-northeast1" }, async (request) => {
    if (!request.auth) {
    ...
    if (!data.type || !data.itemId || !data.warehouseId || !data.quantity) {
        throw new https_1.HttpsError("invalid-argument", "type, itemId, warehouseId, quantity는 필수입니다.");
    }
    if (data.quantity <= 0) {
        throw new https_1.HttpsError("invalid-argument", "수량은 0보다 커야 합니다.");
    }
    // 품목/창고 정보 조회
    ...
    if (!itemSnap.exists) {
        throw new https_1.HttpsError("not-found", "품목을 찾을 수 없습니다.");
    }
    if (!warehouseSnap.exists) {
        throw new https_1.HttpsError("not-found", "창고를 찾을 수 없습니다.");
    }
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (data.type === "incoming" || data.type === "adjustment_plus" || data.type === "return") {
            ...
            newQty = currentQty - data.quantity;
            if (newQty < 0) {
                throw new https_1.HttpsError("failed-precondition", `재고가 부족합니다. 현재: ${currentQty}, 요청: ${data.quantity}`);
            }
        }
        else {
            throw new https_1.HttpsError("invalid-argument", `지원하지 않는 트랜잭션 유형: ${data.type}`);
        }
        // 재고 문서 업데이트 또는 생성
        const inventoryData = {
            itemId: data.itemId,
    ...
    const auditEvent = data.type === "incoming" ? "INVENTORY_IN"
        : data.type === "outgoing" ? "INVENTORY_OUT"
            : "INVENTORY_ADJUST";
    await (0, writeAuditLog_1.writeAuditLogInternal)({
        event: auditEvent,
 ...
 */
function generateLotNo(type, itemCode) {
    const now = new Date();
...
//# sourceMappingURL=processInventoryTx.js.map