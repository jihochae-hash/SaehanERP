 ...
 */
exports.setUserRole = (0, https_1.onCall)({ region: "asia-northeast1" }, async (request) => {
    // 인증 확인
    if (!request.auth) {
    ...
    if (request.auth.token.role !== "ceo") {
        throw new https_1.HttpsError("permission-denied", "CEO만 역할을 변경할 수 있습니다.");
    }
    const { targetUid, role, modules } = request.data;
    if (!targetUid || !role) {
        throw new https_1.HttpsError("invalid-argument", "targetUid와 role은 필수입니다.");
    }
    const level = ROLE_LEVELS[role];
    if (level === undefined) {
        throw new https_1.HttpsError("invalid-argument", `유효하지 않은 역할입니다: ${role}`);
    }
    // Custom Claims 설정
